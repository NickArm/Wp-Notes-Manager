...
class NotesManager {
    ...
    public function addNotesMetaBox() {
        ...
        add_meta_box(
            'wpnm_post_notes',
            esc_html__('Post Notes', 'notes-manager'),
        ...
        add_meta_box(
            'wpnm_page_notes',
            esc_html__('Page Notes', 'notes-manager'),
            [$this, 'renderNotesMetaBox'],
            'page',
            'side',
            'high'
    ...
    public function renderNotesMetaBox($post) {
        ...
        <div id="wpnm-notes-container">
            <div class="wpnm-notes-form">
                <h4><?php esc_html_e('Add New Note', 'notes-manager'); ?></h4>
                <p>
                    <label for="wpnm-note-title"><?php esc_html_e('Title:', 'notes-manager'); ?></label>
                    <input type="text" id="wpnm-note-title" name="wpnm_note_title" class="widefat" />
                </p>
                <p>
                    <label for="wpnm-note-content"><?php esc_html_e('Content:', 'notes-manager'); ?></label>
                    <textarea id="wpnm-note-content" name="wpnm_note_content" rows="3" class="widefat"></textarea>
                </p>
                <p>
                    <label for="wpnm-note-priority"><?php esc_html_e('Priority:', 'notes-manager'); ?></label>
            ...
            <div class="wpnm-notes-list">
                <h4><?php esc_html_e('Existing Notes', 'notes-manager'); ?></h4>
                <?php if (empty($notes)): ?>
    ...
    public function addNotesToAdminBar($wp_admin_bar) {
        ...
        $wp_admin_bar->add_node([
            ...
            'title' => sprintf(esc_html__('Notes (%d)', 'notes-manager'), $notes_count),
            'href' => esc_url(admin_url('admin.php?page=wpnm-dashboard')),
            'meta' => [
                'title' => esc_html__('View Notes Dashboard', 'notes-manager')
            ]
        ]);
    }
    ...
    public function addDashboardWidget() {
        ...
        wp_add_dashboard_widget(
            ...
            esc_html__('Recent Notes', 'notes-manager'),
    ...
    public function renderDashboardWidget() {
        ...
        if (empty($notes)) {
            echo '<p>' . esc_html__('No notes yet.', 'notes-manager') . '</p>';
            echo '<p><a href="' . esc_url(admin_url('admin.php?page=wpnm-dashboard')) . '" class="button">' . esc_html__('Add Note', 'notes-manager') . '</a></p>';
            return;
        }
        ...
        echo '</div>';
        echo '<p><a href="' . esc_url(admin_url('admin.php?page=wpnm-dashboard')) . '" class="button">' . esc_html__('View All Notes', 'notes-manager') . '</a></p>';
    }
    ...
    public function addDeadlineDashboardWidget() {
        ...
        wp_add_dashboard_widget(
            ...
            esc_html__('Upcoming & Overdue Tasks', 'notes-manager'),
    ...
    public function renderDeadlineDashboardWidget() {
        ...
        <div class="wpnm-deadline-widget">
            <div class="wpnm-stats-grid">
                <div class="wpnm-stat-item overdue">
                    <h4><?php esc_html_e('Overdue', 'notes-manager'); ?></h4>
                    <div class="stat-number"><?php echo esc_html($overdue_count); ?></div>
                    <p><?php esc_html_e('Tasks past deadline', 'notes-manager'); ?></p>
                </div>
                
                <div class="wpnm-stat-item upcoming">
                    <h4><?php esc_html_e('Due Soon', 'notes-manager'); ?></h4>
                    <div class="stat-number"><?php echo esc_html($upcoming_count); ?></div>
                    <p><?php esc_html_e('Tasks due in 7 days', 'notes-manager'); ?></p>
                </div>
            </div>
            ...
            <?php if ($overdue_count > 0 || $upcoming_count > 0): ?>
                <div class="wpnm-widget-actions">
                    <a href="<?php echo esc_url(admin_url('admin.php?page=wpnm-all-notes')); ?>" class="button button-primary">
                        <?php esc_html_e('View All Tasks', 'notes-manager'); ?>
                    </a>
                </div>
            <?php else: ?>
                <div class="wpnm-widget-empty">
                    <p><?php esc_html_e('🎉 Great job! No overdue or upcoming tasks.', 'notes-manager'); ?></p>
                    <a href="<?php echo esc_url(admin_url('admin.php?page=wpnm-all-notes')); ?>" class="button">
                        <?php esc_html_e('Create New Task', 'notes-manager'); ?>
                    </a>
                </div>
            <?php endif; ?>
            ...
            <div class="wpnm-widget-footer">
                <small>
                    ...
                    if ($preferences && isset($preferences['deadlines']['enabled'])) {
                        ...
                        printf(esc_html__('Receiving daily notifications for tasks due within %d days.', 'notes-manager'), esc_html($days_ahead));
                    } else {
                        esc_html_e('Configure email notifications in your profile settings.', 'notes-manager');
                    }
                    ?>
                </small>
                ...
                <?php if ($preferences && isset($preferences['deadlines']['enabled'])): ?>
                    ...
                    <button type="button" id="wpnm-test-notification" class="button button-small" style="margin-top: 5px;">
                        <?php esc_html_e('Test Notification', 'notes-manager'); ?>
                    </button>
                <?php endif; ?>
            </div>
        </div>
        ...
        jQuery(document).ready(function($) {
            $('#wpnm-test-notification').on('click', function() {
                ...
                button.text('<?php esc_html_e('Sending...', 'notes-manager'); ?>').prop('disabled', true);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            alert('<?php esc_html_e('Test notification sent successfully! Check your email.', 'notes-manager'); ?>');
                        } else {
                            alert(response.data.message || '<?php esc_html_e('Error sending test notification.', 'notes-manager'); ?>');
                        }
                    },
                    error: function() {
                        alert('<?php esc_html_e('Error sending test notification.', 'notes-manager'); ?>');
                    },
                    complete: function() {
    ...
    public function addNotesColumn($columns) {
        $columns['wpnm_notes'] = esc_html__('Notes', 'notes-manager');
        return $columns;
    }
    ...
    public function showNotesColumn($column_name, $post_id) {
        if ($column_name === 'wpnm_notes') {
            ...
            if ($notes_count > 0) {
                // translators: %d is the number of notes
                echo '<span style="color: #0073aa;">' . esc_html(sprintf(_n('%d note', '%d notes', $notes_count, 'notes-manager'), $notes_count)) . '</span>';
            } else {
                echo '<span style="color: #999;">' . esc_html__('No notes', 'notes-manager') . '</span>';
            }
        }
    }
